...
const EVENTS: SCAEvent[] = [
  // {
  //   id: 'example-event',
  //   title: 'Tech Talk: Breaking into FAANG',
  //   description: 'A panel of BCU alumni share their experiences.',
  //   location: 'Curzon Building, Room G18',
  //   isOnline: false,
  //   date: new Date('2025-11-01T18:00:00'),
...
export default function EventsPage() {
  const upcoming = EVENTS.filter(e => e.date >= new Date()).sort((a, b) => +a.date - +b.date)
      ...
      {upcoming.length === 0 ? (
        ...
        <div className="flex flex-col gap-px border border-[var(--b1)] rounded-xl overflow-hidden">
          {upcoming.map(event => {
            ...
            return (
              <div key={event.id} className="bg-[var(--bg2)] px-4 sm:px-5 py-4 flex items-center gap-3 sm:gap-4 border-b border-[var(--b1)] last:border-b-0 hover:bg-[var(--bg3)] transition-colors">
                {/* Date block */}
                ...
                <div className="flex-1 min-w-0">
                  <div className="text-[13px] font-medium text-[var(--t1)] mb-1.5 truncate">{event.title}</div>
                  <div className="flex gap-2 sm:gap-3 flex-wrap">
                    <span className="text-[11px] text-[var(--t4)] flex items-center gap-1">
                      {event.isOnline ? '⊕' : '◎'} {event.location}
                    </span>
                    <span className="text-[11px] text-[var(--t4)] hidden sm:inline">
